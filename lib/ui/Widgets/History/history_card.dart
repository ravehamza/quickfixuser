...
import 'package:quickfixuser/ui/Pages/mechanic_update_page.dart';
import 'package:quickfixuser/ui/loader_dialog.dart';
...
class _reviewdialogState extends State<reviewdialog> {
  ...
  _addreviewrating(rid, review, rating, mid) async {
   ...
   await Firestore.instance.collection('mechanic').document(mid).get().then((value){
     //Chitya aithy kuj experiement kari
     //((double.parse(value['rating']['value'])*double.parse(value['rating']['count'])+double.parse(rating.toString()))/(double.parse(value['rating']['count'])+1)).toString().isEmpty() ?  {}
